...
function ailand_scripts() {
	...
	*/
	...
	wp_register_script( 'main-js', get_template_directory_uri() . '/assets/js/script-min.js', array(), filemtime( get_template_directory() . '/assets/js/script-min.js' ), true );
...
function ailand_admin_style() {
	wp_enqueue_style( 'main-admin-style', get_template_directory_uri() . '/assets/css/admin.css', array(), false, 'all' );
}
...
<p class="product-text">Darmowa dostawa od 500 zł netto</p>
...
add_filter(
	...
	function ( $address, $raw_address, $order ) {
...
add_action( 'woocommerce_after_quantity_input_field', 'add_unit_label_to_quantity' );
function add_unit_label_to_quantity() {
	echo '<p class="quantity__label">Liczba sztuk</p>';
}
